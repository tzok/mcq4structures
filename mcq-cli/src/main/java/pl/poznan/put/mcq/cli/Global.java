...
public final class Global {
  private static final Options OPTIONS =
      new Options()
          ...
          .addOption(Helper.OPTION_SIMPLE);

  public static void main(final String[] args)
      throws ParseException, PdbParsingException, IOException, IncomparableStructuresException {
    ...
    final MCQGlobalResult mcqResult = (MCQGlobalResult) mcq.compareGlobally(s1, s2);
    final GlobalResult rmsdResult = rmsd.compareGlobally(s1, s2);

    if (commandLine.hasOption(Helper.OPTION_SIMPLE.getOpt())) {
      System.out.println(mcqResult.getMeanDirection().getRadians());
    } else {
