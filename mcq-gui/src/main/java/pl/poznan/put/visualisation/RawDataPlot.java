...
public class RawDataPlot extends AbstractDrawable {
  ...
  public RawDataPlot(final Collection<? extends Angle> data, final double diameter) {
    this(data, diameter, FastMath.PI / 2, FastMath.PI / 12, AngleTransformation.CLOCK);
  }

  public RawDataPlot(final Collection<? extends Angle> data) {
    this(data, 640);
  }
  ...
  public void draw() {
    ...
    final Map<Integer, List<Angle>> observations = new TreeMap<>();

    for (final Angle circular : data) {
      final double degrees = circular.degrees360();
    ...
    for (final Map.Entry<Integer, List<Angle>> entry : observations.entrySet()) {
      ...
      int i = 0;
      for (final Angle circular : entry.getValue()) {
  ...
  public final Collection<Angle> getData() {
