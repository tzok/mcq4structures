...
public class ParallelGlobalComparator extends Thread {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(ParallelGlobalComparator.class);
    ...
    private GlobalResult[][] fillResultsMatrix() {
        ...
        for (int i = 0; i < all; i++) {
            try {
                ...
                results[result.i][result.j] =
                results[result.j][result.i] = result.value;
            } catch (InterruptedException | ExecutionException e) {
                ParallelGlobalComparator.LOGGER
                        .error("Failed to compare a pair of structures", e);
            }
        }
    ...
    public class CompareCallable
            implements Callable<CompareCallable.SingleResult> {
