...
public class FilteredListModel extends AbstractListModel<PdbCompactFragment> {
    ...
    public final List<PdbCompactFragment> getElements() {
        return Stream.concat(listRNAs.stream(), listProteins.stream())
    ...
    public final boolean canAddElement(final PdbCompactFragment element,
                                       final boolean isSizeConstrained) {
        final MoleculeType moleculeType = element.getMoleculeType();
        if ((getSize() > 0) &&
        ...
        if (isSizeConstrained) {
            final List<PdbCompactFragment> list =
                    (moleculeType == MoleculeType.RNA) ? listRNAs
                                                       : listProteins;
            if ((!list.isEmpty()) && (list.get(0).getResidues().size() !=
                                      element.getResidues().size())) {
                return false;
            }
        }
