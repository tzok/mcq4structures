...
/**
 * Class which allow to parse a selection query of the following form
 * CHAIN:NUMBER-ICODE:COUNT e.g. A:1:5.
 */
    ...
    public static SelectionQuery parse(final String query) {
        ...
        if (!matcher.matches() || (matcher.groupCount() != 2)) {
            throw new InvalidQueryException(String.format(
                    ...
                    "followed by optional insertion code: %s", split[1]));
        }
        ...
        try {
            ...
            throw new InvalidQueryException(
                    "Failed to parse given input as " + "integer number", e);
        }
    }

    private SelectionQuery(final String originalQuery,
                           final PdbResidueIdentifier identifier,
                           final int count) {
        super();
    ...
    public PdbCompactFragment apply(final PdbModel model) {
        ...
        if (selectedResidues.size() != count) {
            throw new IllegalArgumentException(
                    "Failed to create selection: " + originalQuery);
        }
