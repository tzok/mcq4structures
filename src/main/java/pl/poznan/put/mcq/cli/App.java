...
public final class App {
  private static final MasterTorsionAngleType DEFAULT_ANGLE_TYPE =
      new AverageTorsionAngleType(MoleculeType.RNA, RNATorsionAngleType.mainAngles());
  ...
  private App(final String[] args)
      throws ParseException, McqProcessingException, IOException, PdbParsingException {
    super();
    ...
    options.addOption(
        ...
        "mode",
        true,
        String.format("(required) mode of operation, one of: %s", Arrays.toString(Mode.values())));
    options.addOption(
        "a",
        "angle",
        true,
        String.format(
            "(optional for %s mode) torsion angle to be used for "
                + "comparison. One of: %s or default: %s",
            Mode.TARGET_MODELS,
    ...
    angleType =
        commandLine.hasOption('a')
            ? RNATorsionAngleType.valueOf(commandLine.getOptionValue('a'))
    ...
    switch (mode) {
      case SINGLE:
        if (arguments.length != 1) {
          throw new McqProcessingException(
              "In SINGLE mode you need to provide exactly 1 " + "structure");
        }
      ...
      case MULTIPLE:
        if (arguments.length < 2) {
          throw new McqProcessingException(
              "In MULTIPLE mode you need to provide at least 2 " + "structures");
        }
      ...
      case TARGET_MODELS:
        if (arguments.length < 2) {
          throw new McqProcessingException(
              "In TARGET_MODELS mode you need to provide at " + "least 2 structures");
        }
        for (final String argument : arguments) {
          loadSingleModel(argument);
        }
  ...
  public static void main(final String[] args)
      throws ParseException, McqProcessingException, PdbParsingException, IOException,
  ...
  private void runSingle() throws IOException, McqProcessingException {
    ...
    final StructureSelection selection = SelectionFactory.create("model", model);
  ...
  private void runMultiple() {
    ...
    final ParallelGlobalComparator.ProgressListener progressListener = new MyProgressListener();
    final ParallelGlobalComparator parallelComparator =
        new ParallelGlobalComparator(comparator, selections, progressListener);
    parallelComparator.start();
  }

  private void runTargetModels()
      throws McqProcessingException, IncomparableStructuresException, IOException {
    sanityCheck();

    final LocalComparator mcq = new MCQ(Collections.singletonList(angleType));
    ...
    for (int i = 0; i < targetFragments.size(); i++) {
      ...
      for (int j = 1; j < selections.size(); j++) {
        ...
        final List<PdbCompactFragment> compactFragments = model.getCompactFragments();
      ...
      final File svgFile =
          File.createTempFile(target.getName() + '_' + Integer.toString(i + 1) + '_', ".svg");
      ...
      System.err.println(
          "Colorbar for fragment " + Integer.toString(i + 1) + " is available here: " + svgFile);

      final File csvFile =
          File.createTempFile(target.getName() + '_' + Integer.toString(i + 1) + '_', ".csv");
      final FileOutputStream stream = new FileOutputStream(csvFile);
      try {
        selectedAngleResults.export(stream);
        System.err.println(
            "Results for fragment " + Integer.toString(i + 1) + " are available here: " + csvFile);
      } finally {
  ...
  private static void drawTorsionAnglesHistograms(final StructureSelection selection)
    ...
    for (final MasterTorsionAngleType masterType : selection.getCommonTorsionAngleTypes()) {
      ...
      try {
        ...
        final File outputFile =
            File.createTempFile("mcq-" + masterType.getExportName() + '-', ".svg");
        final SVGDocument svgDocument = histogram.finalizeDrawing();

        final byte[] bytes = SVGHelper.export(svgDocument, Format.SVG);
        FileUtils.writeByteArrayToFile(outputFile, bytes);
        System.err.println(
            "Histogram for " + masterType.getExportName() + " is available here: " + outputFile);
  ...
  private void sanityCheck() throws McqProcessingException {
    ...
    for (int i = 0; i < targetSize; i++) {
      ...
      for (int j = 1; j < selections.size(); j++) {
        ...
        final int modelFragmentSize = modelFragment.getResidues().size();

        if (targetFragmentSize != modelFragmentSize) {
          throw new McqProcessingException(
              "The size of fragment "
                  + Integer.toString(i + 1)
                  + " does not match for target "
                  + target.getName()
                  + " ("
                  + targetFragmentSize
                  + ") and model "
  ...
  private static class MyProgressListener implements ParallelGlobalComparator.ProgressListener {
