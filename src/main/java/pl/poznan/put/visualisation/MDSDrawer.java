...
public final class MDSDrawer {
    ...
    public static SVGDocument scale2DAndVisualizePoints(
        ...
        return MDSDrawer.scale2DAndVisualizePoints(distanceMatrix,
                                                   MDSDrawer.COLOR_PROVIDER,
                                                   MDSDrawer.NAME_PROVIDER);
    }
    ...
    public static SVGDocument scale2DAndVisualizePoints(
        ...
        for (int i = 0; i < scaledXYMatrix.length; i++) {
            scaledXYMatrix[i][0] = (scaledXYMatrix[i][0] - bounds.getX()) *
                                   (MDSDrawer.DESIRED_WIDTH /
                                    bounds.getWidth());
            scaledXYMatrix[i][1] = (scaledXYMatrix[i][1] - bounds.getY()) *
                                   (MDSDrawer.DESIRED_WIDTH /
                                    bounds.getHeight());
        }
        ...
        for (int i = 0; i < scaledXYMatrix.length; i++) {
            ...
            graphics.draw(new Ellipse2D.Double(x, y, MDSDrawer.CIRCLE_DIAMETER,
        ...
        if (colorMap.size() > 1) {
            ...
            for (final Map.Entry<Color, List<Integer>> entry : colorMap
                    .entrySet()) {
                ...
                final String nameAbbreviated = StringUtils
                        .abbreviate(clusterName, MDSDrawer.MAX_CLUSTER_NAME);

                graphics.setColor(entry.getKey());
                graphics.draw(new Ellipse2D.Double(
                        MDSDrawer.DESIRED_WIDTH + MDSDrawer.CIRCLE_DIAMETER,
                        legendHeight, MDSDrawer.CIRCLE_DIAMETER,
                        MDSDrawer.CIRCLE_DIAMETER));
                graphics.drawString(nameAbbreviated,
                                    (float) (MDSDrawer.DESIRED_WIDTH +
                                             (MDSDrawer.CIRCLE_DIAMETER * 2.5)),
                                    (legendHeight + lineHeight) -
                                    (MDSDrawer.CIRCLE_DIAMETER / 2.0f));
                legendHeight += lineHeight;
                ...
                if (indices.size() == 2) {
                    ...
                    final double x1 = scaledXYMatrix[p1][0] +
                                      (MDSDrawer.CIRCLE_DIAMETER / 2.0);
                    final double y1 = scaledXYMatrix[p1][1] +
                                      (MDSDrawer.CIRCLE_DIAMETER / 2.0);
                    final int p2 = indices.get(1);
                    final double x2 = scaledXYMatrix[p2][0] +
                                      (MDSDrawer.CIRCLE_DIAMETER / 2.0);
                    final double y2 = scaledXYMatrix[p2][1] +
                                      (MDSDrawer.CIRCLE_DIAMETER / 2.0);
                    graphics.draw(new Line2D.Double(x1, y1, x2, y2));
                    continue;
                }

                final Deque<Coord2d> convexHull =
                        MDSDrawer.calculateConvexHull(scaledXYMatrix, indices);
                ...
                final UnivariateFunction functionX =
                        interpolator.interpolate(is, xs);
                final UnivariateFunction functionY =
                        interpolator.interpolate(is, ys);
    ...
    private static Deque<Coord2d> calculateConvexHull(
        ...
        for (int i = 0; i < indices.size(); i++) {
            ...
            points[i] = new Coord2d(scaledXYMatrix[index][0] +
                                    (MDSDrawer.CIRCLE_DIAMETER / 2.0),
                                    scaledXYMatrix[index][1] +
                                    (MDSDrawer.CIRCLE_DIAMETER / 2.0));
        }
    ...
    private static Rectangle2D calculateBounds(
            final double[][] scaledXYMatrix) {
    ...
    public static SVGDocument scale2DAndVisualizePointsOld(
        ...
        final double[][] scaledXYMatrix =
                MDS.multidimensionalScaling(originalDistanceMatrix, 2);
        final double[][] scaledDistanceMatrix = MDSDrawer
    ...
    private static double[][] calculateScaledDistanceMatrix(
            final double[][] originalDistanceMatrix,
            final double[][] scaledXYMatrix) {
    ...
    private static SVGDocument drawPoints(
            final List<? extends NamedPoint> points) {
        final DOMImplementation dom =
                SVGDOMImplementation.getDOMImplementation();
        final SVGDocument document = (SVGDocument) dom
    ...
    private static void createAndAddLinearGradients(final Document document,
                                                    final Node svgRoot,
        ...
        for (final NamedPoint point : points) {
            if (point instanceof ColoredNamedPoint) {
                final Set<Color> colors =
                        ((ColoredNamedPoint) point).getColors();

                if (colors.size() > 1) {
                    final Element linearGradient = document.createElementNS(
                    ...
                    for (final Color color : colors) {
                        final Element stop = document.createElementNS(
                                SVGDOMImplementation.SVG_NAMESPACE_URI,
    ...
    private static void createAndAddTextElements(final Document document,
                                                 final Node svgRoot,
                                                 final List<? extends
                                                         NamedPoint> points) {
        ...
        for (final NamedPoint point : points) {
            ...
            if (point instanceof ColoredNamedPoint) {
                ...
                if (colors.size() == 1) {
                    ...
                    element.setAttributeNS(null,
                                           SVGConstants.SVG_FILL_ATTRIBUTE,
                                           String.format("url(#%s)", StringUtils
                                                   .deleteWhitespace(
                                                           point.getName())));
                }
            }
