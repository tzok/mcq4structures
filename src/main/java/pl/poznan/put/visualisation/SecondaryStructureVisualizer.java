...
public class SecondaryStructureVisualizer {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(SecondaryStructureVisualizer.class);
    private static final BpSeqToDotBracketConverter CONVERTER =
    ...
    public static SVGDocument visualize(FragmentMatch fragmentMatch) {
        ...
        try {
            ...
            dotBracket = SecondaryStructureVisualizer.CONVERTER.convert(bpSeq);
        } catch (InvalidStructureException e) {
            SecondaryStructureVisualizer.LOGGER
                    .error("Failed to extract canonical secondary structure",
                           e);
        ...
        Double[] angleDeltasArray =
                angleDeltas.toArray(new Double[angleDeltas.size()]);
        return SecondaryStructureVisualizer
                .visualize(dotBracket, angleDeltasArray);
    }
    ...
    public static SVGDocument visualize(DotBracket dotBracket,
        ...
        try {
            ...
            double max = 0;
            for (double d : angleDeltas) {
                max = d > max ? d : max;
            }
            ModeleColorMap modelColorMap =
                    ColorMapWrapper.getVarnaColorMap(0, max);
            ...
            Rectangle2D boundingBox =
                    SVGHelper.calculateBoundingBox(svgDocument);
            ...
            return svgDocument;
                ...
                ExceptionFileFormatOrSyntax | ExceptionWritingForbidden |
                IOException e) {
            SecondaryStructureVisualizer.LOGGER
                    .error("Failed to visualize secondary structure:\n"
                           + dotBracket, e);
        } finally {
    ...
    public static SVGDocument visualize(FragmentMatch fragmentMatch,
        ...
        for (ResidueComparison residueComparison : residueComparisons) {
            TorsionAngleDelta angleDelta =
                    residueComparison.getAngleDelta(masterType);
