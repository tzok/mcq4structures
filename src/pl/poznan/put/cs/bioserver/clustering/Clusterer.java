...
public class Clusterer {
    ...
    public static Vector<Vector<Integer>> sClusters;
     ...
     * @param linkage SINGLE, COMPLETE or AVERAGE.
    ...
    public static int[][] hierarchicalClustering(double[][] matrix, int linkage) {
         ...
         */
        Clusterer.sClusters = new Vector<Vector<Integer>>();
        for (int i = 0; i < matrix.length; ++i) {
            Vector<Integer> c = new Vector<Integer>();
            c.add(i);
            Clusterer.sClusters.add(c);
        }

        Vector<int[]> result = new Vector<int[]>();
        while (Clusterer.sClusters.size() > 1) {
            /*
             * get two clusters to be merged
             */
            int[] toMerge = new int[2];
            double leastDiff = Double.POSITIVE_INFINITY;
            for (int i = 0; i < Clusterer.sClusters.size(); ++i) {
                for (int j = i + 1; j < Clusterer.sClusters.size(); ++j) {
                    Vector<Integer> c1 = Clusterer.sClusters.get(i);
                    Vector<Integer> c2 = Clusterer.sClusters.get(j);
                    double delta = 0;
             ...
             * merge clusters
             */
            Vector<Integer> c1 = Clusterer.sClusters.get(toMerge[0]);
            Vector<Integer> c2 = Clusterer.sClusters.get(toMerge[1]);
            c1.addAll(c2);
            Clusterer.sClusters.remove(toMerge[1]);
     ...
     * 
     * @param row Row taken out from distance matrix.
     * @param candidates A set of medoids to be checked.
     * @param exclude One specified medoid to be excluded from search (set to -1
     *            if none should be excluded)
